...
def load_episode_records(root: Path) -> list[dict[str, Any]]:
    ...
    for pq_path in tqdm.tqdm(pq_paths, desc="loading metadata chunks", leave=False):
...
def convert_data(
    ...
    for (chunk_idx, file_idx), records in grouped.items():
        ...
        for record in tqdm.tqdm(
            records,
            desc=f"extracting data (chunk {chunk_idx})",
            leave=False
        ):
...
def convert_videos(
    ...
    for video_key in video_keys:
        ...
        for (chunk_idx, file_idx), records in grouped.items():
            ...
            for record in tqdm.tqdm(
                records,
                desc=f"extracting {video_key} (chunk {chunk_idx})",
                leave=False
            ):
                episode_index = int(record["episode_index"])
...
def convert_episodes_metadata(new_root: Path, episode_records: list[dict[str, Any]]) -> None:
    ...
    with (
        ...
        for record in tqdm.tqdm(sorted(episode_records, key=lambda rec: int(rec["episode_index"])), desc="convert episodes metadata"):
            legacy_episode = {
                key: value
...
def convert_dataset(
    ...
    # 1. Standardize path and validate existence
    root = Path(dataset_path).resolve()
    
    if not root.exists():
        raise FileNotFoundError(f"The local dataset path '{root}' does not exist.")

    # 2. Skip download logic entirely
    validate_local_dataset_version(root)
    logging.info("Using local dataset at %s", root)

    # 3. Setup workspace for conversion
    # We create temporary directories relative to the source data
    episode_records = load_episode_records(root)
    info = load_info(root)
    video_keys = [key for key, ft in info["features"].items() if ft.get("dtype") == "video"]
    chunks_size = info.get("chunks_size", DEFAULT_CHUNK_SIZE)

    backup_root = root.parent / f"{root.name}_backup_v30"
    new_root = root.parent / f"{root.name}_converted_v21"

    # Cleanup old conversion attempts
    ...
    # 5. Swap directories
    # 'root' is renamed to backup, 'new_root' takes the original name
    logging.info("Swapping original data with converted data...")
    shutil.move(str(root), str(backup_root))
    shutil.move(str(new_root), str(root))
    logging.info("Done! Original data backed up at %s", backup_root)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--force-conversion",
        action="store_true",
        help="Required if you want to overwrite previous attempts (handled by script logic).",
    )
...
if __name__ == "__main__":
    init_logging()
    args = parse_args()
    # Note: we pass 'dataset_path' now instead of 'repo_id'
    convert_dataset(dataset_path=args.path, force_conversion=args.force_conversion)