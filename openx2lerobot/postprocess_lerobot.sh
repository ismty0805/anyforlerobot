#!/bin/bash
#SBATCH --job-name=postprocess_lerobot
...
# Usage: sbatch --dependency=afterok:<CONVERSION_JOB_ID> postprocess_lerobot.sh <OUTPUT_NAME>
# Example: sbatch --dependency=afterok:1235 postprocess_lerobot.sh droid_100
...
if [ $# -lt 1 ]; then
    ...
    echo ""
    echo "Arguments:"
    echo "  OUTPUT_NAME : Dataset name (e.g., droid_100, bridge_openx)"
    echo ""
    echo "Example:"
    echo "  sbatch --dependency=afterok:1235 $0 droid_100"
    exit 1
fi
